...
def plotAddedImages():
    circle_image = cv2.imread("bin1.png")
    rectangle_image = cv2.imread("bin2.png")
    added_image = cv2.add(circle_image, rectangle_image)
    added_weight_image = cv2.addWeighted(circle_image, 0.7, rectangle_image, 0.3, 0)

    imgs_array = [circle_image, rectangle_image, added_image, added_weight_image]
    titles_array = ['Circulo', 'Retangulo', 'cv2.add', 'cv2.addWeighted']
    showMultipleImageGrid(imgs_array, titles_array, 2, 2)
